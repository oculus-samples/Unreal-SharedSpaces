...
#include "OvrNetDriver.h"

#include "OvrNetIPAddress.h"
...
bool UOvrNetDriver::IsAvailable() const
{
	return true;
}

ISocketSubsystem* UOvrNetDriver::GetSocketSubsystem()
{
	if (bIsPassthrough)
	{
...
bool UOvrNetDriver::InitBase(bool bInitAsClient, FNetworkNotify* InNotify, const FURL& URL, bool bReuseAddressAndPort, FString& Error)
{
	if (bIsPassthrough)
	{
	...
	{
		UOvrPlatformSubsystem* OvrPlatform = GWorld->GetGameInstance()->GetSubsystem<UOvrPlatformSubsystem>();
		NetworkingConnectionStateChangeDelegateHandle =
			OvrPlatform->GetNotifDelegate(ovrMessage_Notification_Networking_ConnectionStateChange)
			.AddUObject(this, &UOvrNetDriver::OnNetworkingConnectionStateChange);
	}

	return true;
}

bool UOvrNetDriver::InitConnect(FNetworkNotify* InNotify, const FURL& ConnectURL, FString& Error)
{
	UE_LOG(LogNet, Verbose, TEXT("Connecting to host: %s"), *ConnectURL.ToString(true));

	FOvrInternetAddr OvrAddr(ConnectURL);
	if (!OvrAddr.IsValid())
	{
	...
	UOvrNetConnection* Connection = NewObject<UOvrNetConnection>(NetConnectionClass);
	check(Connection);

	// Set it as the server connection before anything else so everything knows this is a client
	ServerConnection = Connection;
	Connection->InitLocalConnection(this, nullptr, ConnectURL, ovr_Net_IsConnected(OvrAddr.GetID()) ? USOCK_Open : USOCK_Pending);

	Connections.Add(OvrAddr.GetID(), Connection);

	// Connect via OVR_Networking
	ovr_Net_Connect(OvrAddr.GetID());
...
bool UOvrNetDriver::InitListen(FNetworkNotify* InNotify, FURL& LocalURL, bool bReuseAddressAndPort, FString& Error)
{
	...
	if (!PeerConnectRequestDelegateHandle.IsValid())
	{
		...
		PeerConnectRequestDelegateHandle =
			OvrPlatform->GetNotifDelegate(ovrMessage_Notification_Networking_PeerConnectRequest)
			.AddUObject(this, &UOvrNetDriver::OnNewNetworkingPeerRequest);
	}
...
void UOvrNetDriver::TickDispatch(float DeltaTime)
{
	if (bIsPassthrough)
	{
	...
	for (;;)
	{
		...
		if (IsServer() && PendingClientConnections.Contains(PeerID))
		{
			...
			TSharedPtr<FInternetAddr> OvrAddr = MakeShareable(new FOvrInternetAddr(PeerID));
			StatelessConnect = StatelessConnectComponent.Pin();

			const ProcessedPacket UnProcessedPacket =
				ConnectionlessHandler->IncomingConnectionless(OvrAddr, Data, PacketSize);

			bool bRestartedHandshake = false;
			bPassedChallenge = !UnProcessedPacket.bError && StatelessConnect->HasPassedChallenge(OvrAddr, bRestartedHandshake);
			...
			if (bPassedChallenge)
			{
				...
				Connection->InitRemoteConnection(this, nullptr, FURL(), *OvrAddr, ovr_Net_IsConnected(PeerID) ? USOCK_Open : USOCK_Pending);
...
void UOvrNetDriver::LowLevelSend(TSharedPtr<const FInternetAddr> Address, void* Data, int32 CountBits, FOutPacketTraits& Traits)
{
	if (bIsPassthrough)
	{
		return UIpNetDriver::LowLevelSend(Address, Data, CountBits, Traits);
	}
	FOvrInternetAddr OvrAddr(FURL(nullptr, *Address->ToString(false), ETravelType::TRAVEL_Absolute));
	ovrID PeerID = OvrAddr.GetID();
...
void UOvrNetDriver::OnNewNetworkingPeerRequest(TOvrMessageHandlePtr Message, bool bIsError)
{
...
bool UOvrNetDriver::AddNewClientConnection(ovrID PeerID)
{
...
void UOvrNetDriver::OnNetworkingConnectionStateChange(TOvrMessageHandlePtr Message, bool bIsError)
{
	auto NetworkingPeer = ovr_Message_GetNetworkingPeer(*Message);
	auto PeerID = ovr_NetworkingPeer_GetID(NetworkingPeer);
...
void UOvrNetDriver::Shutdown()
{
	...
	UE_LOG(LogNet, Verbose, TEXT("Ovr Net Driver shutdown"));
	UOvrPlatformSubsystem* OvrPlatform = GWorld->GetGameInstance()->GetSubsystem<UOvrPlatformSubsystem>();
	if (PeerConnectRequestDelegateHandle.IsValid())
	{
		OvrPlatform->RemoveNotifDelegate(ovrMessage_Notification_Networking_PeerConnectRequest, PeerConnectRequestDelegateHandle);
		PeerConnectRequestDelegateHandle.Reset();
	}
	if (NetworkingConnectionStateChangeDelegateHandle.IsValid())
	{
		OvrPlatform->RemoveNotifDelegate(ovrMessage_Notification_Networking_ConnectionStateChange, NetworkingConnectionStateChangeDelegateHandle);
...
bool UOvrNetDriver::IsNetResourceValid()
{
	if (bIsPassthrough)
	{
